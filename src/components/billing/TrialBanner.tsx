...
export const TrialBanner = () => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 w-[380px] sm:w-[420px] animate-in slide-in-from-bottom-8 fade-in duration-500">
      <div className={`relative overflow-hidden rounded-2xl border shadow-2xl backdrop-blur-xl backdrop-saturate-150 transition-all duration-300 ${
          isExpired || isUrgent 
      ...
      >
        ...
        <button 
          ...
          className={`absolute top-3 right-3 p-1 rounded-full transition-colors z-20 ${
            isExpired || isUrgent ? 'text-red-400 hover:bg-red-100 hover:text-red-600' : 'text-slate-400 hover:bg-white/10 hover:text-white'
          }`}
        >
          <X className="w-4 h-4" />
        </button>
            ...
            <div className="space-y-1 pr-6">
              ...
              <p className={`text-[13px] leading-relaxed ${isExpired || isUrgent ? 'text-red-700/90' : 'text-slate-300'}`}>
                {isExpired ? (
                  ...
                  <span>Full access to grading tools for up to <b className="text-white">{declared_seat_count} students</b>.</span>
                )}
              </p>
            </div>
          </div>
          ...
          <div className={`flex items-center justify-between gap-3 pt-3 border-t ${isExpired || isUrgent ? 'border-red-100' : 'border-white/10'}`}>
            ...
            {!isExpired ? (
              <div className={`flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-md ${
                isUrgent ? 'bg-orange-100 text-orange-700' : 'bg-white/5 text-slate-300 border border-white/5'
              }`}>
