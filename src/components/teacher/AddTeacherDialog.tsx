...
import { Plus, Eye, EyeOff, UserCheck } from "lucide-react";
...
const AddTeacherDialog = ({ onAddTeacher, trigger, children }: AddTeacherDialogProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store the created teacher info for the assignment prompt
      if (result?.id) {
        setCreatedTeacher({
          id: result.id,
      ...
      <AlertDialog open={showAssignPrompt} onOpenChange={setShowAssignPrompt}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <strong>{createdTeacher?.full_name}</strong> has been added as a teacher.
              Would you like to assign them to classes and subjects now?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={handleAssignLater}>
              Assign Later
            </AlertDialogCancel>
            <AlertDialogAction onClick={handleAssignNow} className="bg-primary text-white">
